...
function checkIfCanWin() {
    const arr = JSON.parse(JSON.stringify(board)); //Permite hacer una copia profunda de la informacion en arreglos bidimensionales
...
function checkIfWinner(){
    ...
    const res = [s1, s2, s3, s4, s5, s6, s7, s8].filter(line => {
     ...
     if(res.length > 0) { 
        //Si es mayor que 0 entonces hay un ganador
        if(res[0] === "XXX") {
            playerDiv.textContent = "PC WINS";
            return "pcwon";
        } else {
            playerDiv.textContent = "USTED GANO REY";
            return "userwon";
        }
     } else {
        let draw = true;
...
function bestMove() {
    ...
    board[bestMove.i][bestMove.j] = PCPlays();
    currentPlayer = human;
}
