...
function handleEnter(e, current) {
    if (e.key === "Enter" && e.target.value !== "") {
        const newComment = {
            text: e.target.value, likes: 0, responses: []
        }
...
function renderComments (arr, parent){
    arr.forEach(element => {
        ...
        //append
        commentContainer.appendChild(textContainer);//appendChild inyecta un nuevo elemento
        ...
        if (element.responses.length > 0) { //uso de la recursion
